...
const FormEditTataTertib = ({ goBack }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await axios.get(
        `http://localhost:5000/sub-criteria/${id}`
      );
  ...
  const updateData = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(`http://localhost:5000/sub-criteria/${id}`, {
        code: data.code,
        name: data.name,
        weight: data.weight,
...
export default FormEditTataTertib;
